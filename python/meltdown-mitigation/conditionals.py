...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    """
    pct_value = ((voltage * current) / theoretical_max_power) * 100

    if pct_value >= 80:
        efficiency_band = "green"
    elif 60 <= pct_value < 80:
        efficiency_band = "orange"
    elif 30 <= pct_value < 60:
        efficiency_band = "red"
    else:
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    pct_value = temperature * neutrons_produced_per_second

    if pct_value < (threshold * 0.9):
        status_code = "LOW"
    elif (threshold * 0.9) <= pct_value <= (threshold * 1.1):
        status_code = "NORMAL"
    else:
        status_code = "DANGER"
